var colors = ["#727cf5"];
(dataColors = $("#basic-radar").data("colors")) && (colors = dataColors.split(","));
var options = { chart: { height: 350, type: "radar" }, series: [{ name: "Series 1", data: [80, 50, 30, 40, 100, 20] }], colors: colors, labels: ["January", "February", "March", "April", "May", "June"] };
(chart = new ApexCharts(document.querySelector("#basic-radar"), options)).render();
colors = ["#FF4560"];
(dataColors = $("#radar-polygon").data("colors")) && (colors = dataColors.split(","));
options = {
...
(dataColors = $("#radar-multiple-series").data("colors")) && (colors = dataColors.split(","));
var chart;
options = {
    chart: { height: 350, type: "radar" },
    series: [
