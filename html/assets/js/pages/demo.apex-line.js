var colors = ["#ffbc00"];
(dataColors = $("#line-chart").data("colors")) && (colors = dataColors.split(","));
var options = {
...
colors = ["#39afd1"];
(dataColors = $("#line-chart-annotations").data("colors")) && (colors = dataColors.split(","));
options = {
    annotations: {
        yaxis: [{ y: 8200, borderColor: "#0acf97", label: { borderColor: "#0acf97", style: { color: "#fff", background: "#0acf97" }, text: "Support" } }],
        xaxis: [
            { x: new Date("23 Nov 2017").getTime(), borderColor: "#775DD0", label: { borderColor: "#775DD0", style: { color: "#fff", background: "#775DD0" }, text: "Anno Test" } },
            { x: new Date("03 Dec 2017").getTime(), borderColor: "#ffbc00", label: { borderColor: "#ffbc00", style: { color: "#fff", background: "#ffbc00" }, orientation: "horizontal", text: "New Beginning" } },
        ],
        points: [{
            x: new Date("27 Nov 2017").getTime(),
            y: 8506.9,
            marker: { size: 8, fillColor: "#fff", strokeColor: "#fa5c7c", radius: 2 },
            label: { borderColor: "#fa5c7c", offsetY: 0, style: { color: "#fff", background: "#fa5c7c" }, text: "Point Annotation" },
        }, ],
    },
...
options = {
    chart: { height: 374, type: "line", shadow: { enabled: !1, color: "#bbb", top: 3, left: 2, blur: 3, opacity: 1 } },
    ...
    fill: { type: "gradient", gradient: { shade: "dark", gradientToColors: ["#fa5c7c"], shadeIntensity: 1, type: "horizontal", opacityFrom: 1, opacityTo: 1, stops: [0, 100, 100, 100] } },
...
(chart = new ApexCharts(document.querySelector("#line-chart-gradient"), options)).render();
colors = ["#ffbc00", "#0acf97", "#39afd1"];
(dataColors = $("#line-chart-missing").data("colors")) && (colors = dataColors.split(","));
options = {
    chart: { height: 380, type: "line", zoom: { enabled: !1 }, animations: { enabled: !1 } },
...
(chart = new ApexCharts(document.querySelector("#line-chart-missing"), options)).render();
colors = ["#6c757d", "#0acf97", "#39afd1"];
(dataColors = $("#line-chart-dashed").data("colors")) && (colors = dataColors.split(","));
options = {
    chart: { height: 380, type: "line", zoom: { enabled: !1 } },
    dataLabels: { enabled: !1 },
...
(chart = new ApexCharts(document.querySelector("#line-chart-dashed"), options)).render();
ts2 = 14844186e5;
var data = [];
for (spikes = [5, -5, 3, -3, 8, -8], i = 0; i < 30; i++) {
    innerArr = [(ts2 += 864e5), dataSeries[1][i].value];
    data.push(innerArr);
}
colors = ["#0acf97"];
(dataColors = $("#line-chart-stepline").data("colors")) && (colors = dataColors.split(","));
options = {
...
(chart = new ApexCharts(document.querySelector("#line-chart-stepline"), options)).render();
var lastDate = 0;
data = [];
...
function getNewSeries(e, t) {
    var a = e + 864e5;
    (lastDate = a), data.push({ x: a, y: Math.floor(Math.random() * (t.max - t.min + 1)) + t.min });
}
...
var dataColors;
colors = ["#39afd1"];
(dataColors = $("#line-chart-realtime").data("colors")) && (colors = dataColors.split(","));
var chart;
options = {
    chart: { height: 350, type: "line", animations: { enabled: !0, easing: "linear", dynamicAnimation: { speed: 2e3 } }, toolbar: { show: !1 }, zoom: { enabled: !1 } },
    dataLabels: { enabled: !1 },
